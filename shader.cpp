...
static GLuint loadShader(GLenum type, const char *shaderSrc, const char *name) {
    ...
    if (!compiled) {
        if(type==GL_VERTEX_SHADER)
            LOG(ERROR) <<"Error compiling vtx shader for " << name;
        else 
            LOG(ERROR) <<"Error compiling fgmnt shader for " << name;
        GLint size = 0;
...
GLuint buildShaderProgram(const char* vtxSrc, const char* pxlSrc, const char* name) {
    ...
    GLuint fragmentShader = loadShader(GL_FRAGMENT_SHADER, pxlSrc, name);
    if (fragmentShader == 0) {
